...
runner.update(dataset_generator=dp_relational.data.movies.dataset, n_syn1=3880, n_syn2=6040,
              synth='mst', epsilon=6.0, eps1=2.0, eps2=2.0, k=3, dmax=10, T=Tconst,
...
for loops in range(NUM_LOOPS):
    ...
    #         runner.load_artifacts('db0072ec-bd1b-11ef-8c41-8a7f9025f31e')
    #         results = runner.run(extra_params={ "info": make_summary_dict(), "run_set": "M9L_MediumPGD_eps" })
    ...
    #     runner.regenerate_cross_answers = True
    #     runner.load_artifacts('db0072ec-bd1b-11ef-8c41-8a7f9025f31e')
    #     results = runner.run(extra_params={ "info": make_summary_dict(), "run_set": "M9L_MediumPGD_Tfinal" })
    #     run_count += 1
    #     print(f"eps: {runner.epsilon}, error_ave: {results['error_ave']}, error_max: {100 * np.max(np.array([np.sum(x) for x in results['errors']]))}")
    #     print(f"###### COMPLETED {run_count} RUNS ######")
    reset_runner()
    for g_in in g_rel_opts:
        ...
        runner.regenerate_cross_answers = True
        runner.load_artifacts('db0072ec-bd1b-11ef-8c41-8a7f9025f31e')
        results = runner.run(extra_params={ "info": make_summary_dict(), "run_set": "M9L_MediumPGD_g_in" })
        run_count += 1
        print(f"eps: {runner.epsilon}, error_ave: {results['error_ave']}, error_max: {100 * np.max(np.array([np.sum(x) for x in results['errors']]))}")
        print(f"###### COMPLETED {run_count} RUNS ######")
    reset_runner()
    for q_in in q_reuses:
        ...
        runner.regenerate_cross_answers = True
        runner.load_artifacts('db0072ec-bd1b-11ef-8c41-8a7f9025f31e')
        results = runner.run(extra_params={ "info": make_summary_dict(), "run_set": "M9L_MediumPGD_q_reuse" })
        run_count += 1
        print(f"eps: {runner.epsilon}, error_ave: {results['error_ave']}, error_max: {100 * np.max(np.array([np.sum(x) for x in results['errors']]))}")
        print(f"###### COMPLETED {run_count} RUNS ######")
    reset_runner()
    for k_in in k_news:
        ...
        runner.regenerate_cross_answers = True
        runner.load_artifacts('db0072ec-bd1b-11ef-8c41-8a7f9025f31e')
        results = runner.run(extra_params={ "info": make_summary_dict(), "run_set": "M9L_MediumPGD_knew" })
        run_count += 1
        print(f"eps: {runner.epsilon}, error_ave: {results['error_ave']}, error_max: {100 * np.max(np.array([np.sum(x) for x in results['errors']]))}")
        print(f"###### COMPLETED {run_count} RUNS ######")
    reset_runner()
    for a_in in alphas:
        ...
        runner.regenerate_cross_answers = True
        runner.load_artifacts('db0072ec-bd1b-11ef-8c41-8a7f9025f31e')
        results = runner.run(extra_params={ "info": make_summary_dict(), "run_set": "M9L_MediumPGD_alpha" })
        run_count += 1
        print(f"eps: {runner.epsilon}, error_ave: {results['error_ave']}, error_max: {100 * np.max(np.array([np.sum(x) for x in results['errors']]))}")
        print(f"###### COMPLETED {run_count} RUNS ######")
    reset_runner()