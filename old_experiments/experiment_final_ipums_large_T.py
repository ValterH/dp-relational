...
def cross_generator_torch(qm, eps_rel, T):
    b_round = dp_relational.lib.synth_data.learn_relationship_vector_torch_paper_algo(qm, eps_rel, T=T,
                subtable_size=400000, verbose=True, device=device, queries_to_reuse=6,
                exp_mech_alpha=0.0001, choose_worst=False, slices_per_iter=3, iter_cb=print_iter_eval
                )
    relationship_syn = dp_relational.lib.synth_data.make_synthetic_rel_table_sparse(qm, b_round)
...
while True:
    for T in Ts:
        ...
        results = runner.run(extra_params={ "run_set": 3035, "alpha": 0.2 })
