...
# EPSILON_PYTORCH_EXPERIMENT_DATASET = uuid.UUID('607ae43c-fece-11ee-bd8b-a059507978f3')
...
def cross_generator_torch(qm, eps_rel, T):
    b_round = dp_relational.lib.synth_data.learn_relationship_vector_torch_masked(qm, eps_rel, T=T, T_mirror=150, subtable_size=500000, verbose=True, device=device)
    relationship_syn = dp_relational.lib.synth_data.make_synthetic_rel_table_sparse(qm, b_round)
    return relationship_syn

runner = ModelRunner()
runner.update(dataset_generator=dp_relational.data.movies.dataset, n_syn1=int(7760), n_syn2=int(12080),
              ...
              T=20)
runner.load_artifacts('607ae43c-fece-11ee-bd8b-a059507978f3')

epsilons = [2.5, 3.0, 4.0, 6.0, 8.0, 12.0]
run_count = 0
while True:
    for epsilon in epsilons:
        runner.update(epsilon=epsilon)
        runner.regenerate_qm = True
        results = runner.run(extra_params={ "T_mirror": 150, "run_set": 20 })
