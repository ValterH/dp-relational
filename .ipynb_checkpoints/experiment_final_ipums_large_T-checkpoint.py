...
def cross_generator_torch(qm, eps_rel, T):
    b_round = dp_relational.lib.synth_data.learn_relationship_vector_torch_paper_algo(qm, eps_rel, T=T,
                subtable_size=50000, verbose=True, device=device, queries_to_reuse=None,
                exp_mech_alpha=0.0001, choose_worst=False, slices_per_iter=10, iter_cb=print_iter_eval
                )
    relationship_syn = dp_relational.lib.synth_data.make_synthetic_rel_table_sparse(qm, b_round)
...
runner.update(dataset_generator=lambda dmax: dp_relational.data.ipums.dataset(dmax, frac=0.05, table_cols=['SEX', 'MARST', 'MARRINYR', 'RACASIAN', 'RACWHT']), n_syn1=table_size, n_syn2=table_size,
              synth='mst', epsilon=40.0, eps1=1.0, eps2=1.0, k=3, dmax=4,
              qm_generator=qm_generator_torch, cross_generation_strategy=cross_generator_torch)
# runner.load_artifacts('055bfef0-1590-11ef-9cc2-36406904b081')
...
while True:
    for T in Ts:
        ...
        results = runner.run(extra_params={ "run_set": 3036, "alpha": 0.2 })
