...
def optimal_project_to_simplex_torch(b, m):
    return projection_binarysearch_torch(b[:, 0], m)
...
def learn_relationship_vector_torch_pgd(qm: QueryManagerTorch, epsilon_relationship=1.0, T=100,
    ...
    for t in tqdm(range(T)):
        for x_sli in range(slices_per_iter):
            ...
            # sum corrected slice
            b_adjusted_slice = torch.squeeze(b_slice) * (sub_num_relationships / torch.sum(b_slice))
            b_slice_round = unbiased_sample_torch(b_adjusted_slice, m=sub_num_relationships, device=device)
            rounded_sum = torch.sum(b_slice_round).cpu().numpy()
            if rounded_sum != sub_num_relationships:
                # something went wrong
                print(torch.sum(b_slice))
                print(torch.sum(b_adjusted_slice))
                print(torch.sum(b_slice_round))
                print(sub_num_relationships)
                raise RuntimeError()
